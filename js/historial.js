...
function netoItem(i){
  ...
  if(i.tiretas){
    neto+=num(i.dibujos??i.cantidadDibujo)*EXTRA_DIBUJO_TIRETAS;
    neto+=num(i.cuadrados??i.cantidadCuadrados)*EXTRA_CUADRADOS_TIRETAS;
  }
  return neto;
}
...
function detalleItem(i){
  ...
  if(i.tiretas){
    const dibujos=num(i.dibujos??i.cantidadDibujo);const cuadrados=num(i.cuadrados??i.cantidadCuadrados);
...
function imprimirPresupuesto(p){
  const totals=totalesPresupuesto(p);
  const filas=(p.items||[]).map(i=>{const neto=netoItem(i);const tasa=tasaItem(p,i);const v21=tasa===21?neto*.21:0;const v105=tasa===10.5?neto*.105:0;return `<tr><td class="num">${htmlEscape(num(i.cantidad).toLocaleString('es-AR',{maximumFractionDigits:2}))}</td><td>${htmlEscape(detalleItem(i))}</td><td class="num">${htmlEscape(dinero(i.precioActual??i.precio))}</td><td class="num">${htmlEscape(dinero(neto))}</td><td class="num">${v21?htmlEscape(dinero(v21)):''}</td><td class="num">${v105?htmlEscape(dinero(v105)):''}</td><td class="num">${htmlEscape(dinero(neto+v21+v105))}</td></tr>`;}).join('');
...
function exportarPdf(numero,estado){
  ...
  const {jsPDF}=window.jspdf;const doc=new jsPDF({orientation:'landscape',unit:'mm',format:'a4'});if(typeof doc.autoTable!=='function'){imprimirPresupuesto(p);return;}
  ...
  if(window.CH_LOGO_DATA){try{doc.addImage(window.CH_LOGO_DATA,'PNG',M,10,70,13);}catch(e){}}
  ...
  doc.setFillColor(...light);doc.roundedRect(M,34,W-M*2,20,2,2,'F');
  const meta=[['CLIENTE',p.clienteNombre||'Sin cliente'],['CUIT',p.clienteCuit||'—'],['TELÉFONO',p.clienteTelefono||'—'],['FECHA',fechaAR(p.fecha)]];const widths=[0,126,182,235];meta.forEach((m,idx)=>{const x=M+5+widths[idx];doc.setFontSize(6.5);doc.setTextColor(...gray);doc.setFont('helvetica','bold');doc.text(m[0],x,41);doc.setFontSize(9);doc.setTextColor(...black);doc.text(String(m[1]),x,47,{maxWidth:idx===0?112:46});});
  const filas=(p.items||[]).map(i=>{const neto=netoItem(i),tasa=tasaItem(p,i),v21=tasa===21?neto*.21:0,v105=tasa===10.5?neto*.105:0;return [num(i.cantidad).toLocaleString('es-AR',{maximumFractionDigits:2}),detalleItem(i),dinero(i.precioActual??i.precio),dinero(neto),v21?dinero(v21):'',v105?dinero(v105):'',dinero(neto+v21+v105)];});
  doc.autoTable({startY:59,head:[['Cantidad','Detalle','Unitario','Sub. Total','21%','10,50%','Final c/IVA']],body:filas,theme:'plain',margin:{left:M,right:M,bottom:32},styles:{font:'helvetica',fontSize:8.2,textColor:black,cellPadding:2.4,lineColor:[222,222,222],lineWidth:{bottom:.25}},headStyles:{fillColor:black,textColor:[255,255,255],fontStyle:'bold',fontSize:7.6,cellPadding:3},alternateRowStyles:{fillColor:[250,250,250]},columnStyles:{0:{halign:'right',cellWidth:21},1:{cellWidth:'auto'},2:{halign:'right',cellWidth:31},3:{halign:'right',cellWidth:34},4:{halign:'right',cellWidth:28},5:{halign:'right',cellWidth:28},6:{halign:'right',cellWidth:37}},didDrawPage(){doc.setFillColor(...red);doc.rect(M,H-15,W-M*2,1,'F');doc.setTextColor(...gray);doc.setFontSize(7);doc.setFont('helvetica','normal');doc.text('Ruta 205 km 181,5 · Tel. 2345 498743 · 7260 Saladillo Bs. As.',M,H-9);doc.text(`Página ${doc.internal.getNumberOfPages()}`,W-M,H-9,{align:'right'});}});
  const totals=totalesPresupuesto(p);let y=(doc.lastAutoTable?.finalY||60)+7;if(y>H-47){doc.addPage();y=20;}
  const boxW=82,x=W-M-boxW;doc.setFontSize(8.5);doc.setTextColor(...black);doc.setFont('helvetica','normal');doc.text('Subtotal',x,y+5);doc.text(dinero(totals.subtotal),W-M,y+5,{align:'right'});let yy=y+11;if(totals.iva21){doc.text('IVA 21%',x,yy);doc.text(dinero(totals.iva21),W-M,yy,{align:'right'});yy+=6;}if(totals.iva105){doc.text('IVA 10,5%',x,yy);doc.text(dinero(totals.iva105),W-M,yy,{align:'right'});yy+=6;}doc.setFillColor(...black);doc.roundedRect(x,yy-4,boxW,13,1.5,1.5,'F');doc.setTextColor(255,255,255);doc.setFont('helvetica','bold');doc.setFontSize(10);doc.text('TOTAL',x+5,yy+4);doc.setTextColor(...red);doc.setFontSize(14);doc.text(dinero(totals.total),W-M-4,yy+4,{align:'right'});
