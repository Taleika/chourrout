(function(){
  ...
  function cantidadTexto(v){ return Math.max(0,Math.round(n(v))).toLocaleString('es-AR',{maximumFractionDigits:0}); }
  ...
  function contactoEmpresa(c){
    return [c.empresaDireccion,c.empresaLocalidad,c.empresaTelefono?`Tel. ${c.empresaTelefono}`:'',c.empresaCuit?`CUIT ${c.empresaCuit}`:'',c.empresaEmail].filter(Boolean).join(' · ');
  }
  ...
  function detallePdf(item){
    ...
    if(item.tiretas){
      if(n(item.dibujos)>0)partes.push(`${Math.round(n(item.dibujos))} dibujo${Math.round(n(item.dibujos))===1?'':'s'} artístico${Math.round(n(item.dibujos))===1?'':'s'} por hoja`);
      if(n(item.cuadrados)>0)partes.push(`${Math.round(n(item.cuadrados))} cuadrado${Math.round(n(item.cuadrados))===1?'':'s'}`);
    }
    return partes.join(' · ');
  }
  ...
  async function generarPdfReal(){
    ...
    if(typeof doc.autoTable!=='function'){imprimirComoPdf();return;}
    ...
    doc.setTextColor(...black);doc.setFont('helvetica','bold');doc.setFontSize(17);doc.text('PRESUPUESTO',pageW-margin,16,{align:'right'});
    doc.setTextColor(...gray);doc.setFontSize(9);doc.text(`Nº ${numero}`,pageW-margin,22,{align:'right'});

    doc.setFont('helvetica','bold');doc.setFontSize(8.5);doc.setTextColor(...black);doc.text(c.empresaNombre,margin,36);
    doc.setFont('helvetica','normal');doc.setFontSize(7.5);doc.setTextColor(...gray);
    const contacto=contactoEmpresa(c);
    if(contacto)doc.text(doc.splitTextToSize(contacto,pageW-margin*2-5),margin,40);

    doc.setFillColor(...light);doc.roundedRect(margin,45,pageW-margin*2,17,2,2,'F');
    const info=[['CLIENTE',cliente||'-'],['CUIT',cuit||'-'],['TELÉFONO',telefono||'-'],['FECHA',fechaAR(fecha)||'-']];
    const colW=(pageW-margin*2)/4;
    info.forEach((it,idx)=>{const x=margin+idx*colW+4;doc.setTextColor(...gray);doc.setFont('helvetica','bold');doc.setFontSize(6.8);doc.text(it[0],x,51);doc.setTextColor(...black);doc.setFontSize(9.2);doc.text(String(it[1]),x,57,{maxWidth:colW-8});});

    doc.setFont('helvetica','bold');doc.setFontSize(11);doc.text('Detalle',margin,69);
    doc.setFillColor(...black);doc.roundedRect(pageW-margin-38,64,38,7,3.5,3.5,'F');doc.setTextColor(255,255,255);doc.setFontSize(7.5);doc.text(ivaTexto(),pageW-margin-19,68.7,{align:'center'});

    const filas=items.map(item=>{const neto=calcItem(item),iva=ivaItem(item),v21=iva===21?neto*.21:0,v105=iva===10.5?neto*.105:0;return[cantidadTexto(item.cantidad),detallePdf(item),money(item.precioActual),money(neto),v21?money(v21):'',v105?money(v105):'',money(neto+v21+v105)];});
    doc.autoTable({startY:73,head:[['Cantidad','Detalle','Unitario','Subtotal','21%','10,5%','Final']],body:filas,theme:'plain',margin:{left:margin,right:margin,bottom:35},styles:{font:'helvetica',fontSize:8.2,textColor:black,cellPadding:2.1,valign:'middle',lineColor:[225,225,225],lineWidth:{bottom:.15}},headStyles:{fillColor:black,textColor:[255,255,255],fontStyle:'bold',fontSize:7.4,cellPadding:2.3},alternateRowStyles:{fillColor:[249,249,249]},columnStyles:{0:{halign:'right',cellWidth:20},1:{cellWidth:'auto'},2:{halign:'right',cellWidth:30},3:{halign:'right',cellWidth:32},4:{halign:'right',cellWidth:25},5:{halign:'right',cellWidth:25},6:{halign:'right',cellWidth:34,fontStyle:'bold'}},didDrawPage(){doc.setFont('helvetica','normal');doc.setFontSize(7);doc.setTextColor(...gray);doc.text(c.empresaNombre,margin,pageH-7);const pie=[validezTexto(c),`Página ${doc.internal.getNumberOfPages()}`].filter(Boolean).join(' · ');doc.text(pie,pageW-margin,pageH-7,{align:'right'});}});

    const totals=calcularTotales();
    let y=(doc.lastAutoTable?.finalY||73)+6;
    if(y>pageH-48){doc.addPage();y=18;}
    const boxW=74,boxH=totals.iva21||totals.iva105?28:21,boxX=pageW-margin-boxW;
    doc.setFillColor(...black);doc.roundedRect(boxX,y,boxW,boxH,2.5,2.5,'F');
    doc.setFontSize(8);doc.setTextColor(220,220,220);doc.setFont('helvetica','normal');doc.text('Subtotal',boxX+5,y+7);doc.text(money(totals.subtotal),boxX+boxW-5,y+7,{align:'right'});
    let yy=y+12;
    if(totals.iva21){doc.text('IVA 21%',boxX+5,yy);doc.text(money(totals.iva21),boxX+boxW-5,yy,{align:'right'});yy+=5;}
    if(totals.iva105){doc.text('IVA 10,5%',boxX+5,yy);doc.text(money(totals.iva105),boxX+boxW-5,yy,{align:'right'});yy+=5;}
    doc.setDrawColor(90,90,90);doc.line(boxX+5,yy,boxX+boxW-5,yy);doc.setFont('helvetica','bold');doc.setFontSize(12);doc.setTextColor(255,255,255);doc.text('TOTAL',boxX+5,yy+7);doc.text(money(totals.total),boxX+boxW-5,yy+7,{align:'right'});
  ...
  if(btn){const nuevo=btn.cloneNode(true);btn.parentNode.replaceChild(nuevo,btn);nuevo.addEventListener('click',generarPdfReal);}
})();